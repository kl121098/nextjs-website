import {extractMarkdownData, FrontMatter, getCposts, getOposts} from "@/app/_helpers";
import Link from 'next/link'

const Blogs = async () => {
    const fetchData = async () => {
        const posts = await getOposts();
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {posts.map((post) => (
                    <div key={post.slug} className="bg-white shadow-lg p-4 rounded-md">
                        <img src={post.image} className="h-12 w-12"/>
                        <h1 className="text-xl font-bold mb-2 cursor-pointer mt-4">{post.title}</h1>
    ...
    return (
        <div className="container mx-auto pt-56 pb-24">
            <h1 className="text-3xl font-bold mb-6 text-[#02BDD5] underline">Open Source List</h1>
